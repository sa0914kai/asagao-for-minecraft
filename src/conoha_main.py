...
async def create_vm_from_image(_message):
  await utility.post_message(_message, '> minecraft world opening...')
  # サーバーのVMが存在しているとき実行しない
  await utility.post_message(_message, '> checking...')
  ...
  # VMを作成
  await utility.post_message(_message, '> Start create VM.')
  ...
  try:
    ...
    if response.status_code == 202:
      await utility.post_message(_message, '> Success: Create VM.')
    else:
  ...
  await utility.post_message(_message, '> VM building...')
  wait_time_first = 100
  wait_every_time = 10
  ...
  for i in range(number_of_trials):
    ...
    if servers != None:
      if len(servers) == 0:
        await utility.post_message(_message, '> Failed: VM create failed, server vm is not exist.')
        return None
      server_status = servers[0]['status']
      if server_status == 'ACTIVE':
        await utility.post_message(_message, f'> Done. VM build time = {str(wait_time_first+i*wait_every_time)}(s).')
        break
  ...
  # imageを削除
  await utility.post_message(_message, '> Start remove used image.')
  ...
  for i in range(number_of_trials):
    try:
      ...
      if response.status_code == 204:
        await utility.post_message(_message, f'> Success: image is deleted.')
        break
      else:
        await utility.post_message(_message, f"> [{i}/{number_of_trials}] delete CONOHA_API_IMAGE_SERVICE/v2/images/[image['id']]: {str(response.status_code)}\n\
  ...
  # image削除完了を待つ
  # await utility.post_message(_message, '> Removing image...')
  wait_time_first = 5
  wait_every_time = 5
  number_of_trials = 15
  for i in range(number_of_trials):
    ...
    if images != None:
      if len(images) == 0:
        # await utility.post_message(_message, f'> Removed image done. \n\
...
async def create_image_from_vm(_message):
  await utility.post_message(_message, '> minecraft world closing...')
  ...
  if server['status'] != 'SHUTOFF':
    ...
    try:
      ...
      if response.status_code == 202:
        await utility.post_message(_message, '> Success: stopped VM.')
      else:
        await utility.post_message(_message, f'> post CONOHA_API_COMPUTE_SERVICE/servers/[server_id]: {str(response.status_code)}\n\
          > False: Could not stop.')
        return None
    except requests.exceptions.RequestException as e:
      await utility.post_message(_message, f'> post CONOHA_API_COMPUTE_SERVICE/servers/[server_id]: RequestException')
      return None
  ...
  for i in range(10):
    ...
    if servers != None:
      if len(servers) == 0:
        await utility.post_message(_message, '> Failed: VM shutdown failed, because server not exist.')
        return None
      server_status = servers[0]['status']
      if server_status == 'SHUTOFF':
        await utility.post_message(_message, f'> Done. VM shutdown time = {str(wait_time_first+i*wait_every_time)}(s).')
        break
    time.sleep(wait_every_time)
  if server_status != 'SHUTOFF':
    await utility.post_message(_message, '> VM shutdown failed.')
    return None

  # イメージを作成する
  await utility.post_message(_message, '> Start create Image...')
  wait_time_first = 0
  wait_every_time = 5
  conoha_api_token = await conoha_wrap.get_conoha_api_token(_message)
  if conoha_api_token == None:
  ...
  try:
    ...
    for i in range(number_of_trials):
      response = requests.post(CONOHA_API_COMPUTE_SERVICE+'/servers/'+server_id+'/action', data=json.dumps(data), headers=headers)
      if response.status_code == 202:
        await utility.post_message(_message, '> Success: create Image.')
        break
      else:
        await utility.post_message(_message, f'[{i}/{number_of_trials}] post CONOHA_API_COMPUTE_SERVICE/servers/[server_id]: {response.status_code}.')
      if i == number_of_trials-1:
        return None
      time.sleep(wait_every_time)
  except requests.exceptions.RequestException as e:
  ...
  # Image作成完了まで待機
  await utility.post_message(_message, '> Creating Image...')
  wait_time_first = 70
  wait_every_time = 20
  time.sleep(wait_time_first)
  number_of_trials = 20
  for i in range(number_of_trials):
    ...
    if exist_vm_and_image:
      await utility.post_message(_message, f'> Done.Create image time = {str(wait_time_first+i*wait_every_time)}(s).')
      break
    if i == number_of_trials-1:
  ...
  # VM削除
  await utility.post_message(_message, '> Start remove VM...')
  exist_vm_and_image = await conoha_wrap.exist_both_vm_and_image(_message)
  if exist_vm_and_image != True:
    return None
  wait_time_first = 0
  ...
  try:
    ...
    for i in range(number_of_trials):
      ...
      if response.status_code == 204:
        await utility.post_message(_message, '> Success: Remove VM.')
        break
      else:
  ...
  # VM削除完了を待つ
  await utility.post_message(_message, '> Removing VM...')
  wait_time_first = 5
  wait_every_time = 10
  number_of_trials = 15
  for i in range(number_of_trials):
    ...
    if servers == None:
      continue
    if len(servers) == 0:
      await utility.post_message(_message, f'> Done. Removed VM time = {str(wait_time_first+i*wait_every_time)}(s).')
      break
    if i == number_of_trials-1:
